...
function isSingleExperienceDate(value) {
  if (!value) {
    return false;
  }
...
function splitDateDuration(value) {
  if (!value) {
    return {
      dateRange: null,
      duration: null,
...
function extractEmploymentType(value) {
  if (!value) {
    return null;
  }
...
function extractOrganizationFromMeta(value) {
  if (!value) {
    return null;
  }
...
async function main() {
  ...
  console.log(
    "Open the MAIN LinkedIn profile manually."
  );
  ...
  console.log(
    `Expected profile name: ${expectedName}`
  );

  await sleep(2500);

  console.log(
    "Hydrating main profile..."
  );
  ...
  console.log(
    `Confirmed skills from independent profile signals: ${confirmedSkills.size}`
  );
  ...
  console.log(
    " LINKEDIN EXTRACTION V11 COMPLETE"
  );
  ...
  console.log(
    `Experience roles: ${totalRoles}`
  );

  console.log(
    `  Linked roles: ${experienceResult.debug.linkedRoles}`
  );

  console.log(
    `  DOM fallback roles: ${experienceResult.debug.domFallbackRoles}`
  );
  ...
  if (
    ...
    console.log(
      "DOM-discovered Experience roles:"
    );
  ...
  console.log(
    `Education: ${education.length}${
      expectedEducationCount
        ? ` / expected ${expectedEducationCount}`
        : ""
    }`
  );
  ...
  console.log(
    `Raw candidates: ${skillsResult.rawCandidates.length}`
  );

  console.log(
    `Metadata removed: ${skillsResult.filteredNoise.length}`
  );

  console.log(
    `Final skills: ${skillsResult.skills.length}`
  );

  console.log(
    `Independently confirmed skills: ${confirmedSkills.size}`
  );
