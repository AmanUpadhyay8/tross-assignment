...
test('browser manager uses low-memory production settings', async () => {
  ...
  const manager = new BrowserManager(
    {
      storageStateBase64: Buffer.from(JSON.stringify(storageState)).toString(
        'base64',
      ),
      storageStatePath: '',
    },
...
test('browser manager can offload Chromium to a remote session', async () => {
  ...
  const storageState = {
    cookies: [
      { name: 'session', value: 'private', domain: '.linkedin.com', path: '/' },
    ],
    origins: [
      {
  ...
  assert.equal(await manager.getContext(), context);
  assert.deepEqual(actions, ['route']);
  assert.ok(!actions.includes('close-initial-page'));
  await manager.close();
